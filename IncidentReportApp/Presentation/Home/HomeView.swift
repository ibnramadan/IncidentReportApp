...
struct DatePickerSheet: View {
    ...
    init(title: String, selectedDate: Date, minimumDate: Date? = nil, maximumDate: Date? = nil, onDateSelected: @escaping (Date) -> Void) {
        self.title = title
    ...
    private var dateRange: ClosedRange<Date> {
        let min = minimumDate ?? Calendar.current.date(byAdding: .year, value: -10, to: Date()) ?? Date()
        let max = maximumDate ?? Calendar.current.date(byAdding: .year, value: 1, to: Date()) ?? Date()
        return min...max
    }
}
